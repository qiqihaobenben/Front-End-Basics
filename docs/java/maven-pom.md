...
- **生效范围**：编译、测试、运行时均有效。
- **传递性**：会传递给依赖当前项目的其他项目。
- **类比**：做菜时必须用到的食材（如盐、糖），炒菜、炖汤都要放，做完菜后盘子里也会留着。
- **示例**：
  ```xml
  <dependency>
...
- **传递性**：不会传递给其他项目。
- **类比**：餐厅提供的餐具（如筷子、勺子），你吃饭时要用，但不需要自己带，吃完也不用带走。
- **示例**：
  ```xml
  <dependency>
...
- **传递性**：会传递给其他项目。
- **类比**：手机的充电器，买手机时不需要（已预装系统），但开机后每天都要用。
- **示例**：
  ```xml
  <dependency>
...
- **传递性**：不会传递给其他项目。
- **类比**：考试时的草稿纸，平时学习不需要，只有考试时才用，考完就扔。
- **示例**：
  ```xml
  <dependency>
...
- **传递性**：不会传递给其他项目。
- **类比**：家里的特殊调料（如自制辣椒酱），做菜时要用，但超市买不到，只能自己带。
- **示例**：
  ```xml
  <dependency>
    <groupId>com.example</groupId>
