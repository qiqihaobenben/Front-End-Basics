...
扩展：
为什么 DECIMAL 能精确存储？
核心原理：DECIMAL 是「定点数」，本质是以字符串形式存储整数，只是人为规定小数点位置。

存储机制
1.整数化存储：DECIMAL(10,2) 存储金额 123.45 时，实际存的是整数 12345
2.元数据记录精度：数据库额外存储「小数点位置」信息（这里是 2 位小数）
3.计算时保持整数：123.45 + 67.89 实际计算 12345 + 6789 = 19134，再除以 100 得到 191.34
```
...
| 类型        | 描述                                                        | 范围                                               | 格式                  | 存储空间（字节） | 存储范围                                           |
| ----------- | ----------------------------------------------------------- | -------------------------------------------------- | --------------------- | ---------------- | -------------------------------------------------- |
| `DATE`      | 仅存储日期（年-月-日）。                                    | 1000-01-01 到 9999-12-31                           | `YYYY-MM-DD`          | 3                | 1000-01-01 到 9999-12-31                           |
| `DATETIME`  | 存储日期和时间（年-月-日 时:分:秒），不受时区影响。         | 1000-01-01 00:00:00 到 9999-12-31 23:59:59         | `YYYY-MM-DD HH:MM:SS` | 8                | 1000-01-01 00:00:00 到 9999-12-31 23:59:59         |
| `TIMESTAMP` | 存储时间戳，表示 UTC 时间，常用于记录操作时间，受时区影响。 | 1970-01-01 00:00:01 UTC 到 2038-01-19 03:14:07 UTC | `YYYY-MM-DD HH:MM:SS` | 4                | 1970-01-01 00:00:01 UTC 到 2038-01-19 03:14:07 UTC |
| `TIME`      | 存储时间（时:分:秒）。                                      | -838:59:59 到 838:59:59                            | `HH:MM:SS`            | 3                | -838:59:59 到 838:59:59                            |
| `YEAR`      | 存储年份。                                                  | 1901 到 2155                                       | `YYYY`                | 1                | 1901 到 2155                                       |
