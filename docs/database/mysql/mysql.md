...
### EXISTS 操作符，检查是否存在满足条件的行，如果存在则返回 true，否则返回 false。
SELECT cust_name, cust_contact FROM customers WHERE EXISTS (SELECT * FROM orders WHERE orders.cust_id = customers.cust_id);

### NOT EXISTS 操作符，与 EXISTS 操作符相反，如果存在满足条件的行，则返回 false，否则返回 true。
SELECT cust_name, cust_contact FROM customers WHERE NOT EXISTS (SELECT * FROM orders WHERE orders.cust_id = customers.cust_id);
...
- [JOIN 详解-其中包含子句的执行顺序](https://segmentfault.com/a/1190000015572505)
...
- 在索引全文本数据时，短词被忽略且从索引中排除。短词的定义是那些具有 3 个或 3 个以下字符的词（如果需要，这个数目可以改）
