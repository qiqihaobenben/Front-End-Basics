...
flex 布局是轴导向的，要么是横轴要么竖轴，最多就是可以换行相当于又多了一条轴，因为它的布局只针对于轴线位置，所以把它看做是一维布局。

grid 布局是将 "容器" 划分成表格一样的 "行" 和 "列" ，生成单元格，然后指定 "项目" 所在的单元格，这样就可以把它看作是二维布局。

### 如何决定一个布局应该用 flex 还是 grid？

如果容器内所有子项都是内联展示的，使用 flex 布局可能比较好。

如果容器内有明显的行和列的区分，使用 grid 布局可能比较好。
