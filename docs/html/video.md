...
视频编码和音频编码一般是一起存在的。和视频类似，音频也是从源文件到编码，再到封装。目前使用最广泛的音频编码格式为 ACC，源文件格式为 AAC
...
以上说的是一个完整视频的播放，一个几分钟的完整视频就肯定大于 10M，各大视频网站更多的是几十分钟以上的剧集、综艺，还有大小不固定的直播，肯定不是下载完之后再播放。
...
| 视频封装格式 | flv                             | flv、tag | TS 文件                    | MP4、3GP、webm 切片         |
| 延时         | 低                              | 低       | 高                         | 高                          |
| 数据分段     | 连续流                          | 连续流   | 切片文件                   | 切片文件                    |
| HTML5 播放   | 可通过 HTML5 解包播放（flv.js） | 不支持   | 原生支持，或通过（hls.js） | 直接播放或者 HTML5 解包播放 |
...
两个方案其实只解决了传输问题，例如一个 10S 的视频被分成 10 个 1S 的视频之后，每播放完 1S video 都得重新加载。如果按照这样的方式，实际上每播放 1S，video 上的进度条都会从头跳到尾。代码层面则表现为每过 1S，video 的初始化和结束都会被重新执行，这种体验肯定是无法满足正常使用。

```
HLS (HTTP Live Streaming) - Apple主导
DASH (Dynamic Adaptive Streaming over HTTP) - 国际标准
MSS (Microsoft Smooth Streaming) - 微软方案
RTMP (Real-Time Messaging Protocol) - Adobe，现主要用于推流
```
...
HTTP Live Streaming（HLS），是 Apple 提出的直播流协议，所以在其平台的浏览器上远程就支持。

只用把 HLS 地址直接传递给 video 的 src 属性，浏览器相当于把上述步骤全部完成了。

##### 国产浏览器

而且最开始说过，video 是可替换元素。所以很多浏览器（尤其是国内的移动端浏览器）会使用自己开发过的，不同于标准的浏览器。

- 坏处是不像标准的 video 那么可控。比如无法控制显示层级，永远在页面的最高层等
- 好处是会帮我们处理很多视频甚至流媒体协议
