...
嵌入阶段还有最后一项功能：在获取token的嵌入向量后，它会将token在提示词中的位置编码到嵌入向量中。

简而言之，嵌入向量是 n 维空间中的点，可视作其代表文本的语义含义。在训练过程中，每个token都会在这个空间中移动，以靠近其他相似的token。维度越高，大型语言模型对每个token的表征就越复杂、越细腻。

#### [词向量：AI理解语言的基石](https://mp.weixin.qq.com/s/C8TsMfm4WYuUzvkuRAl90A)
主要介绍了词向量（Word Embedding）技术，将其定位为现代人工智能应用理解语言语义的基石。通过将词汇映射到低维稠密向量空间，让语义相近的词在几何上靠近，从而赋予机器初步的“语义直觉”。

文章还梳理了词向量技术的发展演进：
- 静态嵌入：如Word2Vec、GloVe，特点是“一词一向量”，但无法处理多义词。
- 上下文嵌入：如BERT、RoBERTa，能为同一词汇在不同上下文中生成不同向量，但计算开销大。
- 专用嵌入模型：如通义千问text-embedding-v3、BGE-M3，专为检索、聚类等任务优化，支持指令和多语言，文中特别提到此类开源或可调用的模型适合企业部署。
