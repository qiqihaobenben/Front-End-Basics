...
- SFT (Supervised Fine-Tuning，监督微调)：是一种用于微调大型预训练模型的方法，使用人工标注的“输入-输出”对（提示词 + 理想回答），对预训练模型进行有监督训练。目标是让模型学会遵循指令或模仿特定风格的回复，是最基础的微调方法。
- RLHF (Reinforcement Learning from Human Feedback，基于人类反馈的强化学习)，经典的人类对齐流程：先训练一个奖励模型（模拟人类偏好），然后使用强化学习（通常是 PPO 算法）微调语言模型，让模型生成的回答获得更高的奖励分数。包括：采样 → 奖励打分 → 策略更新 的循环。
- GRPO (Group Relative Policy Optimization，群体相对策略优化)，一种改进的强化学习方法（常见于 DeepSeek 系列模型）。它将 PPO 中的全局价值网络（Critic）替换为组内相对比较：对同一个提示词生成多个回答，以组内平均奖励为基线，减少训练资源消耗，同时提升数学推理等场景下的性能。
- DPO (Direct Preference Optimization，直接偏好优化)，一种较新的偏好对齐方法。不需要像传统强化学习那样先训练奖励模型再在线采样，而是直接用人类偏好数据（“好回答” vs “差回答”）通过特定损失函数一步更新模型，更稳定、计算成本更低。
...
- 遇到新词就 “卡壳”, 一旦碰到训练语料里没见过的词（也就是未记录词, Out-of-Vocabulary），模型就没法识别了
...
BPE（Byte Pair Encoding，字节对编码） 的原理非常直观，可以概括为：在语料库中, 找出频率最高的相邻符号对, 把它们合并为一个新的符号, 然后重复这个过程.
...
嵌入阶段还有最后一项功能：在获取 token 的嵌入向量后，它会将 token 在提示词中的位置编码到嵌入向量中。

简而言之，嵌入向量是 n 维空间中的点，可视作其代表文本的语义含义。在训练过程中，每个 token 都会在这个空间中移动，以靠近其他相似的 token。维度越高，大型语言模型对每个 token 的表征就越复杂、越细腻。

#### [词向量：AI 理解语言的基石](https://mp.weixin.qq.com/s/C8TsMfm4WYuUzvkuRAl90A)

主要介绍了词向量（Word Embedding）技术，将其定位为现代人工智能应用理解语言语义的基石。通过将词汇映射到低维稠密向量空间，让语义相近的词在几何上靠近，从而赋予机器初步的“语义直觉”。

文章还梳理了词向量技术的发展演进：

- 静态嵌入：如 Word2Vec、GloVe，特点是“一词一向量”，但无法处理多义词。
- 上下文嵌入：如 BERT、RoBERTa，能为同一词汇在不同上下文中生成不同向量，但计算开销大。
- 专用嵌入模型：如通义千问 text-embedding-v3、BGE-M3，专为检索、聚类等任务优化，支持指令和多语言，文中特别提到此类开源或可调用的模型适合企业部署。
